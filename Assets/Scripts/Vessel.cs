...
public class Vessel : Body
{
    ...
    public void Move(float input, float mult)
    {
        float desiredAngle = input;
        float rotationOffset = transform.eulerAngles.z;
        Debug.DrawLine(transform.position, transform.position + (Vector3)AngleHelper.DegreesToVector(desiredAngle + rotationOffset) * 2, Color.blue);
        foreach (Thruster t in thrusters)
        {
    ...
    //public void MoveWorld(Vector2 input)
    //{
    //    MoveWorld(input, 1);
    //}

    //public void MoveWorld(Vector2 input, float mult)
    //{
    //    float desiredAngle = AngleHelper.VectorToDegrees(input);
    //    Debug.DrawLine(transform.position, transform.position + (Vector3)AngleHelper.DegreesToVector(desiredAngle) * 2, Color.blue);
    //    foreach (Thruster t in thrusters)
    //    {
    //        float thrusterAngle = AngleHelper.VectorToDegrees(-t.transform.position);
